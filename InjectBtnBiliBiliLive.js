//-------------------------------------------  HEAD VARIABLES  -------------------------------------------
const ExtContainerParentID = "head-info-vm";
var UID = document.location.toString().replace("https://live.bilibili.com/", "");
if (UID.indexOf("?") != -1){
...
//--------------------------------------------- BASIC UTIL ----------------------------------------------------
function SendReg(){
	var title = document.getElementsByClassName("title-length-limit live-skin-main-text v-middle dp-i-block small-title");
	console.log(title);
	/*
...
function OpenSync() {
	if (document.location.toString().indexOf("://live.bilibili.com/") != -1){
...
function MsgNexus(StringData) {
	var NexusParse = StringData.toString().match(/\"Act\":\"MChad-RegOK\"|\"Act\":\"MChad-RollCallApp\"|\"Act\":\"MChad-SetMode\"|\"Act\":\"MChad-PlayApp\"|\"Act\":\"MChad-PauseApp\"|\"Act\":\"MChad-TimeSetApp\"|\"Act\":\"MChad-LiveSend\"|\"Act\":\"MChad-RegListener\"|\"Act\":\"MChad-FilterApp\"|\"Act\":\"MChad-Unsync\"/);
						...
						case ("LiveChat"):
							if (mode < 3){
								...
								*/
								mode = 2;
								btn.textContent = "Synced - Idle";
								spn.textContent = "NOT IMPLEMENTED YET";
							}
							break;
					}
				}
...
function BtnNexus() {
	spn.textContent = "Sorry not working yet";
	/*
	if (mode == 0) {
...
function LoadButtons(ContainerTarget) {
	...
	ContainerTarget.parentNode.insertBefore(ExtContainer, ContainerTarget.nextSibling);
...
async function WaitUntilLoad(){
	...
	while (true){
		...
		var target = document.getElementById(ExtContainerParentID);
		if (target != undefined){
