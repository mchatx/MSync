//-------------------------------------------  HEAD VARIABLES  -------------------------------------------
const ExtContainerParentID = "PlayerContainer";
const TitleCSSSelector = ".VideoTitle";
...
function LatchToVideo() {
	VidSeek = document.getElementById('MainVideoPlayer');
	if (VidSeek == null){
		ws.close();
		spn.textContent = "Can't find Video";
	}

	VidSeek = VidSeek.getElementsByTagName('video');
	if (VidSeek.length == 1){
		MainVid = VidSeek[0];
		if (mode == 6){
...
//--------------------------------------------- BASIC UTIL ----------------------------------------------------
function SendReg(){
	var title = document.getElementsByClassName("VideoTitle")[0];
...
function OpenSync() {
	if (document.location.toString().indexOf("://www.nicovideo.jp/watch/") != -1){
...
function MsgNexus(StringData) {
	var NexusParse = StringData.toString().match(/\"Act\":\"MChad-RegOK\"|\"Act\":\"MChad-RollCallApp\"|\"Act\":\"MChad-SetMode\"|\"Act\":\"MChad-PlayApp\"|\"Act\":\"MChad-PauseApp\"|\"Act\":\"MChad-TimeSetApp\"|\"Act\":\"MChad-RegListener\"|\"Act\":\"MChad-FilterApp\"|\"Act\":\"MChad-PreciseSyncApp\"|\"Act\":\"MChad-PreciseTimeSetApp\"|\"Act\":\"MChad-Unsync\"/);
						...
						case ("LiveChat"):
							if (mode < 3){
								...
								*/
								mode = 2;
								btn.textContent = "Synced - Idle";
								spn.textContent = "THIS IS NOT A LIVE STREAM";
							}
							break;
						case ("SyncTL"):
							if (mode < 3){
			...
			case ("\"Act\":\"MChad-RegListener\""):
				if (mode < 3){
					...
					*/
					mode = 2;
					btn.textContent = "Synced - Idle";
					spn.textContent = "FOR SOME REASON, COMMENTS WON'T LOAD AT ALL (LOOK OVER HERE)--> ";
				}
				break;
...
function LoadButtons(ContainerTarget) {
	...
	ContainerTarget.appendChild(ExtContainer);
...
function StartHereClick(){
	...
	var VideoWrap = document.getElementsByClassName("VideoSymbolContainer");
	if (VideoWrap.length != 0){
		VideoWrap[0].style.zIndex = 1;
	}

	var VidEle = document.getElementById('MainVideoPlayer');
	if (VidEle == null){
		NoticeSpn.textContent = "Can't find Video";
	} else {
		VidEle = VidEle.getElementsByTagName('video');
		if (VidEle.length != 0){
			CaptionDiv.style.backgroundColor = CaptionColour;
			VidEle[0].parentNode.appendChild(CaptionDiv);
...
async function WaitUntilLoad(){
	...
	while (true){
		...
		if (target.length != 0){
			...
			LoadButtons(target[0].parentNode);
			break;
		} else {
